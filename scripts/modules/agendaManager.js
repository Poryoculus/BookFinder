...
export class AgendaManager {
    ...
    try {
      ...
      if (stored) {
        this.agenda = {
          ...
          ...stored,
          readingGoals: Array.isArray(stored.readingGoals)
            ? stored.readingGoals
            : [],
          toReadList: Array.isArray(stored.toReadList) ? stored.toReadList : [],
          currentlyReading: Array.isArray(stored.currentlyReading)
            ? stored.currentlyReading
            : [],
          finishedBooks: Array.isArray(stored.finishedBooks)
            ? stored.finishedBooks
            : [],
          readingStats: {
  ...
  exportData() {
    // Simple implementation - return reading list data
    return {
  ...
  getUserPreferences() {
    // Return basic user preferences
    return {
      favoriteGenres: [],
      readingPace: "medium",
      preferredPageCount: null,
    };
  }
  ...
  addToReadingList(bookData, priority = "medium") {
    ...
    if (existingIndex === -1) {
      ...
      this.agenda.toReadList[existingIndex] = {
        ...this.agenda.toReadList[existingIndex],
        ...bookItem,
        id: this.agenda.toReadList[existingIndex].id,
      };
      this.saveToStorage();
  ...
  startReading(bookItemId, currentPage = 0) {
    const bookIndex = this.agenda.toReadList.findIndex(
      (item) => item.id === bookItemId,
    );
    if (bookIndex === -1) return false;

    const book = this.agenda.toReadList.splice(bookIndex, 1)[0];
    const currentlyReadingItem = {
      ...book,
  ...
  finishReading(bookItemId, rating = null, review = "", finishDate = null) {
    const index = this.agenda.currentlyReading.findIndex(
      (item) => item.id === bookItemId,
    );
    ...
    const finishedBook = {
      ...book,
      finishDate: finishDate || new Date().toISOString(),
      rating,
      review,
      isRead: true,
      readingTime: this.calculateTotalReadingTime(book.readingSessions),
    };
  ...
  updateFinishedBookStats(book) {
    ...
    if (new Date(book.finishDate).getFullYear() === currentYear)
  ...
  calculateWeeklyReadingTime() {
    ...
    const recentSessions = this.agenda.currentlyReading.flatMap((book) =>
      book.readingSessions.filter((s) => new Date(s.date) >= oneWeekAgo),
    );
    return recentSessions.reduce(
      (total, s) => total + (s.minutesSpent || 0),
      0,
    );
  }
  ...
  calculateReadingStreak() {
    const sessions = [
      ...this.agenda.currentlyReading.flatMap((b) => b.readingSessions),
      ...this.agenda.finishedBooks.flatMap((b) => b.readingSessions),
    ];
  ...
  calculateFavoriteGenres() {
    const counts = {};
    this.agenda.finishedBooks.forEach((book) =>
      book.categories?.forEach((cat) => (counts[cat] = (counts[cat] || 0) + 1)),
    );
  ...
  calculateGoalEndDate(timeframe) {
    ...
    switch (timeframe) {
      ...
      case "quarter":
        return new Date(now.getFullYear(), now.getMonth() + 3, 0).toISOString();
      case "year":
      default:
  ...
  checkGoalsCompletion() {
    ...
    this.agenda.readingGoals.forEach((goal) => {
      if (!goal.isCompleted) {
        const booksThisPeriod = this.agenda.finishedBooks.filter(
          (book) =>
            new Date(book.finishDate) >= new Date(goal.startDate) &&
            new Date(book.finishDate) <= new Date(goal.endDate),
        ).length;
  ...
  getBooksByStatus(status) {
    switch (status) {
      case "toRead":
        return this.agenda.toReadList;
      case "reading":
        return this.agenda.currentlyReading;
      case "finished":
        return this.agenda.finishedBooks;
      default:
        return [];
    }
  }
  ...
  getActiveGoals() {
    const now = new Date();
    return this.agenda.readingGoals.filter(
      (goal) => !goal.isCompleted && new Date(goal.endDate) > now,
    );
  }
    ...
    return JSON.stringify(
      {
        ...
        metadata: {
          totalBooks:
            this.agenda.toReadList.length +
            this.agenda.currentlyReading.length +
            this.agenda.finishedBooks.length,
  ...
  importAgendaData(data) {
    try {
      ...
      if (parsed.version && parsed.agenda)
        this.agenda = { ...this.getDefaultAgenda(), ...parsed.agenda };
      this.saveToStorage();
      return true;
    } catch {
      return false;
